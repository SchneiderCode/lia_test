...
//TODO - update to include a "value" option. For most questions this will be the date completed. But for others (like closing comment) it will be their actual feedback.
    try {
...
window.user_name="no_username"
window.user_email="no_email"
...
<script input="submit" default="Waiting for Data" style="display:block; text-align:center;"  >
  if(user_email.endsWith(".edu") || user_email.endsWith(".gov")){
    sendData({username: user_name, email: user_email, course:"NEW_USER", question:"START"})
    "Information Sent"
  }
  else{
...
if(check == 2){
  sendData({username: user_name, email: user_email, course:"NEW_USER", question:"HPC_CLUSTERS"})
  send.lia("true")
} else { send.lia("")}
